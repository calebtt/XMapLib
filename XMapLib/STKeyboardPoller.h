...
namespace sds
{
	...
	struct STKeyboardPoller : public sds::STRunner::DataWrapper
	{
	public:
		using StateType = XINPUT_KEYSTROKE;
	...
	private:
		...
		const KeyboardPlayerInfo &m_local_player;
		...
		StateType tempState{};
	public:
		virtual ~STKeyboardPoller() override
		{
		}
		explicit STKeyboardPoller(const KeyboardPlayerInfo& pl = {}, const KeyboardSettings& sett = {}, const LogFnType fn = nullptr)
		...
		virtual void operator()() override
		{
			...
			auto addElement = [&](const XINPUT_KEYSTROKE& state)
			{
				...
				if (m_protectedData.size() < m_settings.MAX_STATE_COUNT)
					...
					LogFn("STKeyboardPoller::operator()()::addElement(): State buffer dropping states.");
			};
