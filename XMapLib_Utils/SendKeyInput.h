...
namespace sds::Utilities
{
	...
	inline UINT SendVirtualKey(const auto vk, const bool isKeyboard, const bool sendDown) noexcept;
	inline void SendScanCode(const int virtualKeycode, const bool doKeyDown) noexcept;
	inline auto UnsetNumlockAsync() noexcept -> std::optional<std::shared_future<std::string>>;
	[[nodiscard]] inline auto GetScanCode(const int vk) noexcept -> std::optional<WORD>;
	inline void UnsetAndCheckNumlock(const std::chrono::milliseconds timeoutTime = std::chrono::milliseconds{ 20 });
	 ...
	 */
	inline
	void UnsetAndCheckNumlock(const std::chrono::milliseconds timeoutTime)
	{
